...
from backend.nodes.planning_node import planning_node
from backend.nodes.writing_node import writing_node


class ScriptwritingInputState(TypedDict):
    instruction : str
    content : str
    host_name : str
    guest_name : str
...
class ScriptwritingState(TypedDict):
    instruction : str
    title : str
    plan : List[dict]
    dialogue : List[dict]
...
def create_scriptwriting_graph():
    """
    生成對話腳本
    """
    builder = StateGraph(ScriptwritingState, input=ScriptwritingInputState)
    ...
    builder.add_edge("planning_node", "writing_node")
...
if __name__ == "__main__":
    ...
    scriptwriting_workflow = create_scriptwriting_graph()
    result = scriptwriting_workflow.invoke(
